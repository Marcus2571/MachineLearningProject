<template>
  ...
  // Define acceleration and deceleration rates (experiment with different values)
  ...
  class Car {
    ...
    crossCheckPoint(track) {
      ...
      carSuccessfullyTurnedCorner = false;
      if (checkPointId != null) {
        if (this.currentCheckPoint + 1 === checkPointId) {
          this.currentCheckPoint++;
          points.value++;
          carSuccessfullyTurnedCorner = true;
  ...
  // Choice B
  let action;
  if (typeof getRandomAction === 'function') {
  ...
  let carSuccessfullyTurnedCorner = false;
  ...
  async function trainModel() {
    ...
    model.add(tf.layers.dense({ units: 32, inputShape: [3] })); // input shape is the size of the state
    model.add(tf.layers.dense({ units: 32 }));
    model.add(tf.layers.dense({ units: 10, activation: 'softmax' })); // output size is the size of the action
    model.compile({ loss: 'categoricalCrossentropy', optimizer: 'adam' });
  
    const xs = tf.tensor2d(trainingData.map(data => data.state));
    const ys = tf.tensor2d(trainingData.map(data => data.action));
  ...
  //collectData
  function collectData(car, track) {
    // Kør en runde
    for (let t = 0; t < numTimeSteps; t++) {
      // Få bilens nuværende tilstand
      let state = [car.position, car.speed, car.direction];
      // Få den handling, der blev taget i denne tilstand
      let action = getRandomAction();
      let reward = 0;
      if (carSuccessfullyTurnedCorner) { // replace with your condition
        reward = 1;
        console.log(reward)
    ...
    async function gameLoop() {
      ...
      if (trainingData.length >= 10000) {
        tf.tidy(() => {
          ...
          xs = tf.tensor2d(stateData);
          ys = tf.tensor2d(actionData);
  
          const model = tf.sequential();
          model.add(tf.layers.dense({ units: 32, inputShape: [3] })); // input shape is the size of the state
          model.add(tf.layers.dense({ units: 32 }));
          model.add(tf.layers.dense({ units: 3, activation: 'softmax' })); // output size is the size of the action
          model.compile({ loss: 'categoricalCrossentropy', optimizer: 'adam' });
